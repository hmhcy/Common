...
public class FTPUtils {
	...
	public String genFileName(String originalFilename) {
		...
		String uuidString = IDUtils.genID();
		sysFilename = uuidString + suffix;
		return sysFilename;
	}
	/**
	 * 
	 ...
	 * @return
	 */
	public List<String> fileUpload(String path, List<FileSystemResource> files) {
		FTPClient ftp = new FTPClient();
		path = path + "/" + DateUtils.getCurrDate("YYYYMM");
	...
	public boolean removeFile(String remotePath) {
		...
		try {
			...
			if (!result) {
				logger.info("ftp连接失败,hostname:" + host);
			} else {
	...
	public boolean downloadFile(String filename, String localPath) {
		...
		try {
			...
			// 判断是否存在目录
			if (!ftp.changeWorkingDirectory(path)) {
				throw new RuntimeException("文件路径不存在：" + path);
			}
			// 获取该目录所有文件
	...
	// //上传测试----------------------------------- /*FileInputStream in;
	// try {
	// in=new FileInputStream(new
	// File("C:\\Users\\Administrator\\Desktop\\json.png"));
